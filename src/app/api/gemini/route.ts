...
CRITICAL: 응답은 반드시 유효한 JSON 형식이어야 합니다. 다른 형식은 절대 사용하지 마세요.

요구사항:
1. 제목 없이 개요부터 시작
2. 개요: 제안자 수, 의견 수, 최근 날짜만 간단히
3. 카테고리별로 의견이 많은 순서대로 정렬
4. 각 카테고리는 핵심 내용만 2-3개 불릿으로 요약
5. 카테고리 간에만 구분선(---) 사용
6. 개별 의견 목록은 포함하지 않음

JSON 출력 형식 (이 형식을 정확히 따라주세요):
{
...
- 마크다운이나 코드 블록(```)은 사용하지 마세요  
- summary 필드의 값은 문자열이어야 합니다
- 줄바꿈은 \\n으로 표현하세요
